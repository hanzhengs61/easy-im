...
type (
	...
	Users struct {
		...
		Password  string `db:"password"`   // bcrypt 加密后的密码
		Nickname  string `db:"nickname"`   // 昵称
		Avatar    string `db:"avatar"`     // 头像URL
		Status    int64  `db:"status"`     // 状态：1正常 2禁用
		CreatedAt int64  `db:"created_at"` // 创建时间（Unix 时间戳）
		UpdatedAt int64  `db:"updated_at"` // 更新时间（Unix 时间戳）
	}
)
...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Username, data.Password, data.Nickname, data.Avatar, data.Status)
	return ret, err
}

func (m *defaultUsersModel) Update(ctx context.Context, newData *Users) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, usersRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.Username, newData.Password, newData.Nickname, newData.Avatar, newData.Status, newData.Id)
	return err
}
